...
def compute_the_mean(name, path_csv_rd):
    column_names = []
...
def write_into_csv(path_faces_personX, path_csv):
    ...
    with open(path_csv, "w", newline="") as csvfile:
        ...
        for i in range(len(dir_pics)):
            ...
            features_128d = return_128d_features(path_faces_personX + "/" +
                                                 dir_pics[i])
            #  print(features_128d)
            # 遇到没有检测出人脸的图片跳过
            if features_128d == 0:
...
def calculate():
    ...
    # 存放所有特征均值的 CSV 的路径
    path_csv_feature_all = "data/faces_database.csv"
    ...
    with open(path_csv_feature_all, "w", newline="") as csvfile:
        ...
        print(
            "##### 得到的特征均值 / The generated average values of features stored in: #####"
        )

        for i in range(len(csv_rd)):
            feature_mean = compute_the_mean(csv_rd[i].split('.')[0], path_csv_rd + csv_rd[i])
            #print(feature_mean)
